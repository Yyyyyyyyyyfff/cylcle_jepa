...
class BackwardPredictor(nn.Module):
    """
    Backward Predictor for Cycle-JEPA.
    Input: ŷ from forward predictor [B, N_target, embed_dim]
    Output: ŝx [B, N_total, embed_dim]

    Simple design: MLP + repeat to expand from N_target to N_total.
    """
    ...
    def forward(self, y_pred, target_num_patches=None):
        """
        :param y_pred: output from forward predictor [B, N_target, embed_dim]
        :param target_num_patches: N_total from target encoder output (optional, defaults to self.num_patches)
        :return: reconstructed representation [B, target_num_patches, embed_dim]
        """
        B, N_target, _ = y_pred.shape

        if target_num_patches is None:
            target_num_patches = self.num_patches

        y = self.predictor_embed(y_pred)

        if not self.use_rope:
            if N_target != target_num_patches:
                y = torch.nn.functional.interpolate(
                    y.permute(0, 2, 1),
                    size=target_num_patches,
                    mode="linear",
                    align_corners=False,
                ).permute(0, 2, 1)

            # Handle pos_embed: repeat if needed, then truncate
            if target_num_patches > self.predictor_pos_embed.shape[1]:
                repeat_times = (
                    target_num_patches // self.predictor_pos_embed.shape[1]
                ) + 1
                pos_embed = self.predictor_pos_embed.repeat(1, repeat_times, 1)[
                    :, :target_num_patches, :
                ]
            else:
                pos_embed = self.predictor_pos_embed[:, :target_num_patches, :]
            y = y + pos_embed

        for blk in self.predictor_blocks:
            if self.use_activation_checkpointing:
                y = torch.utils.checkpoint.checkpoint(
                    blk, y, None, None, use_reentrant=False
                )
            else:
                y = blk(y, mask=None, attn_mask=None)

        y = self.predictor_norm(y)
        y = self.predictor_proj(y)

        return y


def vit_backward_predictor(**kwargs):
    model = BackwardPredictor(
...
class VisionTransformerPredictor(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.is_video:
            ...
            self.num_patches = num_patches = (img_size[0] // patch_size) * (
                img_size[1] // patch_size
            )
        # Position embedding
    ...
    def _init_pos_embed(self, pos_embed):
        ...
        )  # TODO: update; currently assumes square input
        if self.is_video:
            grid_depth = self.num_frames // self.tubelet_size
            sincos = get_3d_sincos_pos_embed(
                embed_dim,
        ...
        for i, blk in enumerate(self.predictor_blocks):
            if self.use_activation_checkpointing:
                x = torch.utils.checkpoint.checkpoint(
                    blk, x, masks, None, use_reentrant=False
                )
            else:
