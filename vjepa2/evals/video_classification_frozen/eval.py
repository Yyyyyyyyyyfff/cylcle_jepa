...
def main(args_eval, resume_preempt=False):
    ...
    if opt_kwargs is None:
        opt_kwargs = [
            dict(
                weight_decay=args_opt.get("opt_kwargs", {}).get("weight_decay", 0.01),
                final_weight_decay=args_opt.get("opt_kwargs", {}).get(
                    "final_weight_decay", 0.01
                ),
                start_lr=args_opt.get("opt_kwargs", {}).get(
                    "start_lr", args_opt.get("opt_kwargs", {}).get("lr", 0.001)
                ),
                lr=args_opt.get("opt_kwargs", {}).get("lr", 0.001),
                final_lr=args_opt.get("opt_kwargs", {}).get("final_lr", 0.0),
                warmup=args_opt.get("opt_kwargs", {}).get("warmup", 1.0),
            )
        ]
    else:
        opt_kwargs = [
            dict(
                ...
                final_weight_decay=kwargs.get("final_weight_decay", 0.01),
                start_lr=kwargs.get("start_lr", kwargs.get("lr", 0.001)),
                lr=kwargs.get("lr", kwargs.get("start_lr", 0.001)),
                final_lr=kwargs.get("final_lr", 0.0),
                warmup=kwargs.get("warmup", 1.0),
            )
    ...
    # Use DistributedDataParallel only when world_size > 1
    if world_size > 1:
        classifiers = [
...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_bfloat16):
            ...
            # DEBUG: print labels and feature shapes
            if itr == 0:
                logger.info(
                    f"DEBUG labels: min={labels.min().item()}, max={labels.max().item()}, shape={labels.shape}"
                )
            ...
            with torch.no_grad():
                ...
                if itr == 0:
                    logger.info(
                        f"DEBUG encoder output: {type(outputs)}, len={len(outputs)}"
                    )
                    logger.info(f"DEBUG encoder output[0]: shape={outputs[0].shape}")
                if not training:
                    outputs = [[c(o) for o in outputs] for c in classifiers]
            if training:
                outputs = [[c(o) for o in outputs] for c in classifiers]
                if itr == 0:
                    logger.info(f"DEBUG classifier output: shape={outputs[0][0].shape}")

            # Compute loss (inside autocast for bfloat16 to work correctly)
            losses = [[criterion(o, labels) for o in coutputs] for coutputs in outputs]

        # Compute accuracy (outside autocast and no_grad)
        with torch.no_grad():
            outputs = [
        ...
        if training:
            if use_bfloat16:
                scaler[0].scale(losses[0][0]).backward()
        ...
        param_groups = [
            {
                ...
                "mc_start_lr": kwargs.get("start_lr", kwargs.get("lr", 0.001)),
                "mc_ref_lr": kwargs.get("lr", kwargs.get("start_lr", 0.001)),
                "mc_final_lr": kwargs.get("final_lr", 0.0),
                "mc_ref_wd": kwargs.get("weight_decay", 0.01),
                "mc_final_wd": kwargs.get("final_weight_decay", 0.01),
            }
        ]
