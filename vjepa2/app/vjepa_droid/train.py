...
def main(args, resume_preempt=False):
    ...
    temporal_tokens = max_num_frames // tubelet_size
    ...
    free_port = 37200 + hash(socket.gethostname()) % 1000
    world_size, rank = init_distributed(rank_and_world_size=(0, 1), port=free_port)
    logger.info(f"Initialized (rank/world-size) {rank}/{world_size}")

    # Force single GPU mode if NCCL fails
    use_ddp = True
    if world_size == 1:
        use_ddp = False
        logger.info("Single process mode, skipping DDP")
    ...
    if os.path.exists(latest_path):
        (
            ...
            scaler_backward=scaler_backward,
        )
    ...
    def save_checkpoint(epoch, path):
        ...
        if backward_predictor is not None:
            save_dict["backward_predictor"] = backward_predictor.state_dict()
            save_dict["opt_backward"] = optimizer_backward.state_dict()
            save_dict["scaler_backward"] = (
                None if scaler_backward is None else scaler_backward.state_dict()
            )
        try:
            ...
            def train_step():
                ...
                def forward_target(c):
                    with torch.no_grad():
                        ...
                        h = target_encoder(c)
                        h = h.view(batch_size, max_num_frames, -1, h.size(-1)).flatten(
                            1, 2
                        )
                        if normalize_reps:
                ...
                def forward_predictions(z):
                    ...
                    # Official code: no downsampling, actions/states stay as-is
                    _z, _a, _s, _e = (
                        z[:, :-tokens_per_frame],
                        actions,
                        states[:, :-1],
                    ...
                    _z = torch.cat(
                        [z[:, :tokens_per_frame], z_tf[:, :tokens_per_frame]], dim=1
                    )
                ...
                def cycle_loss_fn(s_x, h):
                    h_detached = h.detach()
                    return (
                        torch.mean(torch.abs(s_x - h_detached) ** loss_exp) / loss_exp
                    )

                # Step 1. Forward
                with torch.cuda.amp.autocast(dtype=dtype, enabled=mixed_precision):
                    # Use encoder for predictions
                    z = forward_context(clips)
                    z_tf, z_ar = forward_predictions(z)

                    # Use target_encoder for targets
                    with torch.no_grad():
                        h = forward_target(clips)

                    jloss = loss_fn(z_tf, h)
                    sloss = loss_fn(z_ar, h)
                    loss_forward = jloss + sloss

                    loss_cycle = 0.0
                    if use_cycle_loss and backward_predictor is not None:
                        # z_tf shape: [B, N, D], need to pass target_num_patches
                        target_num_patches = (
                            h.size(1) // tokens_per_frame * tokens_per_frame
                        )
                        s_x = backward_predictor(
                            z_tf, target_num_patches=target_num_patches
                        )
                ...
                if use_cycle_loss and backward_predictor is not None:
                    # encoder + predictor: α*L_forward + (1-α)*L_cycle
                    # backward_predictor: L_cycle only

                    # First: compute loss_combined and backward for all
                    if mixed_precision:
                    ...
                    # Zero gradients for backward_predictor in main optimizer
                    # (since it will be updated separately)
                    if backward_predictor is not None:
                    ...
                    # Second: re-compute loss_cycle with detached inputs to isolate backward_predictor gradients
                    # (avoiding the "modified by inplace" error)
                    if backward_predictor is not None:
                        # Detach to avoid graph issues
                        z_tf_detached = z_tf.detach()
                        h_detached = h.detach()

                        # Re-compute backward predictor output
                        target_num_patches = (
                            h.size(1) // tokens_per_frame * tokens_per_frame
                        )
                        s_x_detached = backward_predictor(
                            z_tf_detached, target_num_patches=target_num_patches
                        )

                        # Re-compute loss_cycle
                        loss_cycle_new = cycle_loss_fn(s_x_detached, h_detached)

                        # Backward for backward_predictor only
                        if mixed_precision:
                            scaler_backward.scale(loss_cycle_new).backward()
                            scaler_backward.unscale_(optimizer_backward)
                        else:
                            loss_cycle_new.backward()
                        if mixed_precision:
